...
def test_column_viewer_accessible_interaction_contract():
    ...
    for token in ('id="province-select"', 'id="band-controls"', 'id="overlay-controls"', 'id="seed-status"', 'id="neighborhood-canvas"', 'id="footprint-canvas"', 'id="footprint-depth-profile"', 'id="footprint-profile-label"', 'id="footprint-map-label"', 'id="footprint-map-legend"', 'id="footprint-passport"', 'data-neighborhood-mode="depth"', 'data-neighborhood-mode="source"', 'data-footprint-profile="volume"', 'data-footprint-profile="seafloor"', 'data-footprint-map="family"', 'data-footprint-map="area"', 'data-footprint-map="volume"', 'data-footprint-map="depth"', 'aria-live="polite"', 'role="img"', "Evidence boundary", "not a province mean", 'src="neighborhoods.js"', 'src="province-footprints.js"'):
        assert token in html
    for token in ("history.replaceState", 'event.key === "Enter"', "aria-pressed", "updateReadout", "renderNeighborhood", "renderFootprint", "renderPassport", "ranksFor", "rankColor", "wet_area_fraction_by_seafloor_band", "water_volume_fraction_by_depth_band", "sampled_water_volume_km3", "not a detected physical regime", "does not reach this band", "bathymetry-truncated part", "not province-wide", "not the province", "overlaySentence"):
        assert token in app
