"""Fetch one native ORAS5 Arctic-entrances T/S/U/V monthly state subset."""
...
def fetch(month: str, mesh_receipt: pathlib.Path, output: pathlib.Path, receipt: pathlib.Path, retrieved_at: str, base: str = BASE) -> dict:
    ...
    with netCDF4.Dataset(output, "w", format="NETCDF4") as target:
        target.createDimension("z", nz)
        target.createDimension("y", ny)
        target.createDimension("x", nx)
        target.setncattr("month", month)
        target.setncattr("mesh_sha256", mesh["output"]["sha256"])
        target.setncattr("boundary", "Native monthly Arctic T/S/U/V subset; no accepted transport or heat-delivery result.")
        for name, array in arrays.items():
    ...
    result = {
        "schema": "oceanlines.oras5.arctic-entrances-state-subset.v1",
        "status": "native_monthly_arctic_state_subset_not_yet_sectioned",
        "month": month,
        ...
        "next_test": "extract Fram and both Barents face paths with signed velocity and adjacent-cell T/S collocation",
        "boundary": (
            "One monthly assimilative-reanalysis member on the native grid. This is not an "
            "observation-only field, climatology, accepted section transport, convergence, or Arctic heat budget."
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("--mesh-receipt", type=pathlib.Path, default=pathlib.Path("research/osw-m3-oras5-arctic-entrances-mesh.json"))
    parser.add_argument("--output", type=pathlib.Path, default=pathlib.Path("atlas/data/oras5-arctic-entrances-state-201802.nc"))
    parser.add_argument("--receipt", type=pathlib.Path, default=pathlib.Path("research/osw-m3-oras5-arctic-entrances-state-201802.json"))
